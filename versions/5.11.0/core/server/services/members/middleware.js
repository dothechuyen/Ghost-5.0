...
const getMemberData = async function (req, res) {
    try {
        const member = await membersService.ssr.getMemberDataFromSession(req, res);
        if (member) {
            let memberData = formattedMemberResponse(member);
            res.json(memberData);
        } else {
            res.json(null);
        }
...
const getIntegrationMemberJWT = async function (req, res) {
    try {
        const member = await membersService.ssr.getMemberDataFromSession(req, res);

        if (member) {
            const integrationJwtUrl = req.headers.integrationjwturl || req.headers.integrationJwtUrl;
            const cookies = req.headers.cookie;
            const body = { ghostCookies: cookies };
            let jwt = await httpPost(integrationJwtUrl, body);
            res.send(jwt);
        } else {
            res.send(null);
        }
...
module.exports = {
    ...
    getMemberData,
    updateMemberData,
    getIntegrationMemberJWT: getIntegrationMemberJWT,
