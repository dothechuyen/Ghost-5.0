...
module.exports = function apiRoutes() {
    ...
    router.post("/settings/routes/yaml", mw.authAdminApi, apiMw.upload.single("routes"), apiMw.upload.validation({ type: "routes" }), http(api.settings.upload));
    ...
    router.post("/users/pwd-reset-token", mw.authAdminApi, http(api.users.pwdResetToken));
    router.post("/users/passwordreset-token", mw.authAdminApi, http(api.users.getResetToken));
    router.post("/users", mw.authAdminApi, http(api.users.add));
    ...
    router.post("/members/upload", mw.authAdminApi, apiMw.upload.single("membersfile"), apiMw.upload.validation({ type: "members" }), http(api.members.importCSV));

    router.get("/members/stripe_connect", mw.authAdminApi, http(api.membersStripeConnect.auth));
    router.get("/get-magic-link", mw.authAdminApi, async (req, res) => {
        try {
            const email = req.body.email;
            const url = await membersService.api.getMagicLink(email);
            return res.json({
                message: "Get Magic Link Successfully !",
                url: url,
            });
        } catch (error) {
    ...
    router.post("/themes/upload", mw.authAdminApi, apiMw.upload.single("file"), apiMw.upload.validation({ type: "themes" }), http(api.themes.upload));
    ...
    router.post("/db", mw.authAdminApi, apiMw.upload.single("importfile"), apiMw.upload.validation({ type: "db" }), http(api.db.importContent));
    ...
    router.post("/session", shared.middleware.brute.globalBlock, shared.middleware.brute.userLogin, http(api.session.add));
    ...
    // ## Authentication
    // router.post("/authentication/password_reset", shared.middleware.brute.globalReset, shared.middleware.brute.userReset, http(api.authentication.generateResetToken));
    router.post("/authentication/passwordreset", shared.middleware.brute.globalReset, shared.middleware.brute.userReset, http(api.authentication.generateResetToken));
    ...
    // ## Images
    router.post("/images/upload", mw.authAdminApi, apiMw.upload.single("file"), apiMw.upload.validation({ type: "images" }), apiMw.normalizeImage, http(api.images.upload));

    // ## media
    router.post("/media/upload", mw.authAdminApi, apiMw.upload.media("file", "thumbnail"), apiMw.upload.mediaValidation({ type: "media" }), http(api.media.upload));
    router.put("/media/thumbnail/upload", mw.authAdminApi, apiMw.upload.single("file"), apiMw.upload.validation({ type: "images" }), http(api.media.uploadThumbnail));

    // ## files
    router.post("/files/upload", mw.authAdminApi, apiMw.upload.single("file"), http(api.files.upload));
    ...
    router.post("/redirects/upload", mw.authAdminApi, apiMw.upload.single("redirects"), apiMw.upload.validation({ type: "redirects" }), http(api.redirects.upload));
